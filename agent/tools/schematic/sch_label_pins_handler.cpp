...
std::string SCH_LABEL_PINS_HANDLER::GenerateLabelPinsCode( const nlohmann::json& aInput ) const
{
    ...
    code << "\n";
    code << "            # Overlap detection with justification swap fallback\n";
    ...
    code << "                    if _conflict:\n";
    code << "                        _alt_h = HA_LEFT if h_align == HA_RIGHT else HA_RIGHT\n";
    code << "                        _lbl._proto.text.attributes.horizontal_alignment = _alt_h\n";
    code << "                        sch.crud.update_items(_lbl)\n";
    code << "                        _bb = sch.transform.get_bounding_box(_lbl, units='mm')\n";
    code << "                        if _bb:\n";
    code << "                            _new_bbox = {'min_x': _bb['min_x'] + _LABEL_SHRINK, 'max_x': _bb['max_x'] - _LABEL_SHRINK, 'min_y': _bb['min_y'] + _LABEL_SHRINK, 'max_y': _bb['max_y'] - _LABEL_SHRINK}\n";
    code << "                            _conflict = _has_real_overlap(_new_bbox)\n";
    code << "                        if not _conflict:\n";
    code << "                            _flipped = True\n";
    code << "                    if _conflict:\n";
    ...
    code << "                    _conflict = _has_real_overlap(_new_bbox)\n";
    code << "                    if _conflict:\n";
    code << "                        # Try swapping horizontal alignment (flips text to other side of pin)\n";
    code << "                        _alt_h = HA_LEFT if h_align == HA_RIGHT else HA_RIGHT\n";
    code << "                        _lbl._proto.text.attributes.horizontal_alignment = _alt_h\n";
    code << "                        sch.crud.update_items(_lbl)\n";
    code << "                        _bb = sch.transform.get_bounding_box(_lbl, units='mm')\n";
