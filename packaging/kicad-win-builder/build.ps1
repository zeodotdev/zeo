...
# Stage kipy (KiCad Python bindings) + runtime deps into the bundled Python's
# site-packages. zeo-mcp.exe and the in-app agent shell both do
# `from kipy.mcp.server import run` on startup; without this step the
# installer ships an empty Lib/site-packages and users get the opaque MCP
# -32000 error on first launch (this is how 0.2.2 shipped — the kipy install
# was historically done by the dev-only rebuild_agent.ps1 and had to be run
# manually after build.ps1).
#
# Inline-only, no shared helper script — keeps the release pipeline
# self-contained. Hard-fails on any error; no silent stderr suppression.
# Smoke-tests the import chain at the end so a half-broken bundle fails the
# build, not the user.
function Install-Kipy-Bundle {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$True)]
        [string]$destBin,
    ...
    $zeoRoot        = Resolve-Path (Join-Path $PSScriptRoot "..\..")
    $kicadPythonDir = Join-Path $zeoRoot "src\zeo-python"
    $kipySource     = Join-Path $kicadPythonDir "kipy"
    $kicadSourceDir = Join-Path $zeoRoot "src\zeo"
    $protoInput     = Join-Path $kicadSourceDir "api\proto"
    $sitePackages   = Join-Path $destBin "Lib\site-packages"

    if (-not (Test-Path $kipySource)) {
        Write-Error "kipy source not found at $kipySource. Refusing to ship an installer without kipy."
        Exit [ExitCodes]::KipyInstallFailure
    }

    if (-not (Test-Path $protoInput)) {
        Write-Error "Zeo proto sources not found at $protoInput"
        Exit [ExitCodes]::KipyInstallFailure
    }

    if (-not (Test-Path $protoc)) {
        Write-Error "protoc not found at $protoc (vcpkg protobuf package may not be installed)"
        Exit [ExitCodes]::KipyInstallFailure
    }

    # Generate protobuf Python files. Always regenerate so stale bindings against
    # newer .proto definitions don't ImportError at runtime.
    Write-Host "Generating kipy protobuf bindings..." -ForegroundColor Cyan
    ...
    Pop-Location
    if ($protoExit -ne 0) {
        Write-Error "Proto generation failed (exit $protoExit)"
        Exit [ExitCodes]::KipyInstallFailure
    }

    # Generate kipy/kicad_api_version.py from `git describe`. Mirrors
    # src/zeo-python/build.py's wheel-build step; we bypass build.py here so
    # we have to replicate it. Without this,
    ...
    $gitDescribe = (& git -C $kicadSourceDir describe --long 2>$null)
    if ([string]::IsNullOrWhiteSpace($gitDescribe)) {
        $shortSha = (& git -C $kicadSourceDir rev-parse --short HEAD 2>$null)
        if ([string]::IsNullOrWhiteSpace($shortSha)) {
            $shortSha = "unknown"
        }
        $gitDescribe = "0.0.0-0-g$($shortSha.Trim())"
    }
...
"@ | Set-Content -Path $apiVersionFile -Encoding utf8
    ...
    # Stage the kipy package into site-packages. Always replace so an
    # incremental build picks up source changes.
    Write-Host "Staging kipy package into site-packages..." -ForegroundColor Cyan
    ...
    # Install kipy's runtime dependencies. Unconditional so this doesn't
    # short-circuit on a stale partial install (that's how 0.2.2 shipped with
    # kipy/ present but google/, pynng/ and sniffio/ missing). Hard-fail
    # rather than silently swallow the error.
    Write-Host "Installing kipy runtime dependencies..." -ForegroundColor Cyan
    & $pythonBin -m pip install --no-user --target $sitePackages --upgrade `
        "protobuf>=6.33" "pynng>=0.8.0,<0.9.0" typing_extensions "mcp>=1.0.0" "Pillow>=10.0"
    if ($LASTEXITCODE -ne 0) {
        Write-Error "pip install of kipy runtime dependencies failed (exit $LASTEXITCODE)"
        Exit [ExitCodes]::KipyInstallFailure
    }

    # Smoke test: confirm the staged bundle actually imports. Mirrors the
    # import chain zeo-mcp.exe and the agent's run_shell do on launch — any
    # missing module (pynng, google.protobuf, sniffio, anyio, etc.) fails
    # the build now instead of the user later.
...
"@
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "kipy smoke test failed — staged site-packages at $sitePackages is not importable. Refusing to package."
        Exit [ExitCodes]::KipyInstallFailure
    }
    Write-Host "  kipy bundle staged and verified." -ForegroundColor Green
}
...
function Start-Prepare-Package {
    ...
    if( -not $lite )
    {
        ...
        # Stage kipy + runtime deps into the bundled Python's site-packages.
        # Only runs when target arch matches host (pip runs against the bundled
        # python). Cross-compile builds emit a warning so the operator knows
        # to run rebuild_agent.ps1 against the staged tree before packaging.
        if( $arch -eq (Get-HostArch) ) {
            $protoc = Join-Path -Path $vcpkgInstalledRootPrimary -ChildPath "tools\protobuf\protoc.exe"
            Install-Kipy-Bundle -destBin $destBin -pythonBin $pythonBin -protoc $protoc
        } else {
            Write-Host "Cross-compile build (target $arch on host $(Get-HostArch)): kipy bundle install skipped." -ForegroundColor Yellow
            Write-Host "  Run packaging\kicad-win-builder\rebuild_agent.ps1 against $destBin before packaging the installer." -ForegroundColor Yellow
        }
