...
PANEL_STARTWIZARD_PRIVACY_BASE::PANEL_STARTWIZARD_PRIVACY_BASE( wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name ) : wxPanel( parent, id, pos, size, style, name )
{
	...
	m_stIntro = new wxStaticText( sbSizer1->GetStaticBox(), wxID_ANY, _("Allow connections to the Internet to check for updated versions of Zeo and packages installed through the Plugin and Content Manager?"), wxDefaultPosition, wxDefaultSize, 0 );
	m_stIntro->Wrap( 550 );
	sbSizer1->Add( m_stIntro, 0, wxALL|wxEXPAND, 5 );

	m_cbAutoUpdateKiCad = new wxCheckBox( sbSizer1->GetStaticBox(), wxID_ANY, _("Check for Zeo updates on startup"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	m_stIntroDataCollection = new wxStaticText( m_sizerDataCollection->GetStaticBox(), wxID_ANY, _("Zeo can anonymously report crashes and other event data to the development team in order to help identify and fix bugs and improve performance.\n\nWhen data collection is enabled, Zeo will automatically send reports when crashes or other notable events occur. These reports contain technical details about the state of the Zeo software at the time of the report. No personally identifiable information (PII) is collected, and your design files such as schematics, PCBs, and libraries are never shared as part of this process."), wxDefaultPosition, wxDefaultSize, 0 );
