...
PANEL_STARTWIZARD_LIBRARIES_BASE::PANEL_STARTWIZARD_LIBRARIES_BASE( wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name ) : wxPanel( parent, id, pos, size, style, name )
{
	...
	m_rbDefaultTables = new wxRadioButton( this, wxID_ANY, _("Start with the built-in Zener libraries (recommended)"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	m_rbImport->SetToolTip( _("Library tables will be imported from the version you selected on the previous step.  Any tables that cannot be imported will be created with the default Zener libraries.") );
