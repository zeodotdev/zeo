    ...
    m_eventCounterTimer( nullptr )
{
    ...
    m_aboutTitle = _HKI( "Zeo PCB Editor" );
...
void PCB_EDIT_FRAME::OnBoardItemRemoved( BOARD& aBoard, BOARD_ITEM* aBoardItem )
{
    ...
    if( m_agentChangeTracker->IsTracked( aBoardItem->m_Uuid ) )
    {
        ...
        // Notify agent frame to re-check pending changes
        std::string payload;
...
void PCB_EDIT_FRAME::OnBoardItemsRemoved( BOARD& aBoard, std::vector<BOARD_ITEM*>& aBoardItems )
{
    ...
    for( BOARD_ITEM* item : aBoardItems )
    {
        ...
        {
            wxLogInfo( "PCB: Tracked item %s removed by user, untracking", item->m_Uuid.AsString() );
...
bool PCB_EDIT_FRAME::DetectAgentChanges()
{
    ...
    for( int i = baseline; i < currentUndoCount; i++ )
    {
        ...
        for( unsigned int j = 0; j < undoCommand->GetCount(); j++ )
        {
            ...
            {
                wxLogWarning( "PCB: Exception accessing item in undo command, skipping" );
            }
        }
    }
