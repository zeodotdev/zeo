                                                    ...
                                                    const nlohmann::json& aInput ) const
{
    ...
    code << "import json, re, fnmatch, sys\n"
         ...
         << "        }\n"
         << "    elif hasattr(info, 'pins') and info.pins:\n"
         << "        _pxs = [getattr(p, 'position_x', 0) / 1_000_000 for p in info.pins]\n"
         << "        _pys = [getattr(p, 'position_y', 0) / 1_000_000 for p in info.pins]\n"
         << "        _w = max(_pxs) - min(_pxs)\n"
         << "        _h = max(_pys) - min(_pys)\n"
         << "        _pad = 1.27\n"
         << "        result['body_size'] = {\n"
         << "            'width': round(_w + 2 * _pad, 2),\n"
         << "            'height': round(_h + 2 * _pad, 2),\n"
         << "        }\n"
         << "    if include_pins:\n"
         << "        pins = []\n"
