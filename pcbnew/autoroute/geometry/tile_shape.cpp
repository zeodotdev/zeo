...
//-----------------------------------------------------------------------------
// HALF_PLANE Implementation
//-----------------------------------------------------------------------------

HALF_PLANE::HALF_PLANE( const VECTOR2I& aPointA, const VECTOR2I& aPointB ) :
    m_point( aPointA ),
    m_dir( aPointB - aPointA )
{
}


HALF_PLANE::HALF_PLANE( const SEG& aSeg ) :
    m_point( aSeg.A ),
    m_dir( aSeg.B - aSeg.A )
{
}
...
bool HALF_PLANE::Contains( const VECTOR2I& aPt ) const
{
    return SideOf( aPt ) >= 0;
}


int64_t HALF_PLANE::SideOf( const VECTOR2I& aPt ) const
{
    // Cross product of direction and point-to-point vector
    // Positive = left side (inside), Negative = right side (outside)
    VECTOR2I toPoint = aPt - m_point;
    return (int64_t)m_dir.x * toPoint.y - (int64_t)m_dir.y * toPoint.x;
}


bool HALF_PLANE::IsParallel( const HALF_PLANE& aOther ) const
{
    // Two lines are parallel if their direction vectors are parallel
    // Cross product is zero for parallel vectors
    int64_t cross = (int64_t)m_dir.x * aOther.m_dir.y - (int64_t)m_dir.y * aOther.m_dir.x;
    return cross == 0;
}


std::optional<VECTOR2I> HALF_PLANE::IntersectionPoint( const HALF_PLANE& aOther ) const
{
    // Solve for intersection using line equations
    // Line 1: m_point + t * m_dir
    // Line 2: aOther.m_point + s * aOther.m_dir

    int64_t denom = (int64_t)m_dir.x * aOther.m_dir.y - (int64_t)m_dir.y * aOther.m_dir.x;

    if( denom == 0 )
        return std::nullopt;  // Parallel lines

    VECTOR2I diff = aOther.m_point - m_point;

    // t = (diff x aOther.m_dir) / denom
    int64_t tNum = (int64_t)diff.x * aOther.m_dir.y - (int64_t)diff.y * aOther.m_dir.x;

    // Calculate intersection point
    // Using fixed-point math to avoid floating point
    int64_t x = m_point.x + ( tNum * m_dir.x ) / denom;
    int64_t y = m_point.y + ( tNum * m_dir.y ) / denom;
...
SIMPLEX::SIMPLEX( const std::vector<HALF_PLANE>& aHalfPlanes ) :
    m_halfPlanes( aHalfPlanes )
{
    computeVertices();
}


SIMPLEX SIMPLEX::FromBox( const BOX2I& aBox )
{
    std::vector<HALF_PLANE> planes;
    planes.reserve( 4 );

    VECTOR2I tl( aBox.GetX(), aBox.GetY() );
    VECTOR2I tr( aBox.GetRight(), aBox.GetY() );
    VECTOR2I br( aBox.GetRight(), aBox.GetBottom() );
    VECTOR2I bl( aBox.GetX(), aBox.GetBottom() );

    // Create half-planes for each edge (counterclockwise order)
    planes.emplace_back( tl, tr );  // Top
    planes.emplace_back( tr, br );  // Right
    planes.emplace_back( br, bl );  // Bottom
    planes.emplace_back( bl, tl );  // Left

    return SIMPLEX( planes );
}


SIMPLEX SIMPLEX::FromHalfSpace( const SEG& aBoundary, const BOX2I& aBoardBounds )
{
    std::vector<HALF_PLANE> planes;

    // The main boundary line
    planes.emplace_back( aBoundary );

    // Add board bounds as limiting planes
    VECTOR2I tl( aBoardBounds.GetX(), aBoardBounds.GetY() );
    VECTOR2I tr( aBoardBounds.GetRight(), aBoardBounds.GetY() );
    VECTOR2I br( aBoardBounds.GetRight(), aBoardBounds.GetBottom() );
    VECTOR2I bl( aBoardBounds.GetX(), aBoardBounds.GetBottom() );

    planes.emplace_back( tl, tr );
    planes.emplace_back( tr, br );
    planes.emplace_back( br, bl );
    planes.emplace_back( bl, tl );

    return SIMPLEX( planes );
}


void SIMPLEX::computeVertices()
{
    m_vertices.clear();

    int n = m_halfPlanes.size();
    if( n < 3 )
        return;

    // Compute vertices as intersection points of consecutive half-planes
    for( int i = 0; i < n; ++i )
    {
        int j = ( i + 1 ) % n;
        auto intersection = m_halfPlanes[i].IntersectionPoint( m_halfPlanes[j] );

        if( intersection )
        {
            // Verify this vertex is inside all other half-planes
            bool valid = true;
            for( int k = 0; k < n && valid; ++k )
            {
                if( k != i && k != j )
                {
                    if( !m_halfPlanes[k].Contains( *intersection ) )
                        valid = false;
                }
            }
...
BOX2I SIMPLEX::BoundingBox() const
{
    if( m_vertices.empty() )
        return BOX2I();
    ...
    return BOX2I( minPt, VECTOR2L( maxPt.x - minPt.x, maxPt.y - minPt.y ) );
}


bool SIMPLEX::Contains( const VECTOR2I& aPt ) const
{
    for( const auto& hp : m_halfPlanes )
    {
        if( !hp.Contains( aPt ) )
            return false;
    }
...
bool SIMPLEX::Intersects( const TILE_SHAPE& aOther ) const
{
    BOX2I otherBox = aOther.BoundingBox();
    BOX2I thisBox = BoundingBox();
...
std::unique_ptr<TILE_SHAPE> SIMPLEX::Intersection( const TILE_SHAPE& aOther ) const
{
    // For simplex-to-box intersection
    const INT_BOX* otherBox = dynamic_cast<const INT_BOX*>( &aOther );
    if( otherBox )
    {
        // Create a simplex from the other box and intersect
        SIMPLEX otherSimplex = SIMPLEX::FromBox( otherBox->BoundingBox() );
        SIMPLEX result = IntersectionSimplex( otherSimplex );
        if( !result.IsEmpty() )
            return std::make_unique<SIMPLEX>( result );
    }

    // For simplex-to-simplex intersection
    const SIMPLEX* otherSimplex = dynamic_cast<const SIMPLEX*>( &aOther );
    if( otherSimplex )
    {
        SIMPLEX result = IntersectionSimplex( *otherSimplex );
        if( !result.IsEmpty() )
...
std::vector<SEG> SIMPLEX::GetEdges() const
{
    std::vector<SEG> edges;
    int n = m_vertices.size();
...
int SIMPLEX::DistanceToEdge( const VECTOR2I& aPt ) const
{
    int minDist = std::numeric_limits<int>::max();
...
VECTOR2I SIMPLEX::NearestPointOnBoundary( const VECTOR2I& aPt ) const
{
    int minDist = std::numeric_limits<int>::max();
    VECTOR2I nearest = aPt;
...
std::unique_ptr<TILE_SHAPE> SIMPLEX::Clone() const
{
    return std::make_unique<SIMPLEX>( *this );
}


void SIMPLEX::Inflate( int aAmount )
{
    // Offset each half-plane outward
    VECTOR2I center = Center();

    for( auto& hp : m_halfPlanes )
    {
        // Move the point outward along the normal
        VECTOR2I normal( -hp.Direction().y, hp.Direction().x );  // Perpendicular
        int64_t len = std::sqrt( (int64_t)normal.x * normal.x + (int64_t)normal.y * normal.y );

        if( len > 0 )
        {
            // Normalize and scale by amount
            VECTOR2I newPoint = hp.Point();
            newPoint.x += ( normal.x * aAmount ) / len;
            newPoint.y += ( normal.y * aAmount ) / len;
            hp = HALF_PLANE( newPoint, newPoint + hp.Direction() );
        }
    }
...
SIMPLEX SIMPLEX::RemoveBorderLine( int aEdgeIndex ) const
{
    if( aEdgeIndex < 0 || aEdgeIndex >= (int)m_halfPlanes.size() )
        return *this;

    std::vector<HALF_PLANE> newPlanes;
    newPlanes.reserve( m_halfPlanes.size() - 1 );

    for( int i = 0; i < (int)m_halfPlanes.size(); ++i )
    {
        if( i != aEdgeIndex )
            newPlanes.push_back( m_halfPlanes[i] );
    }

    return SIMPLEX( newPlanes );
}


SIMPLEX SIMPLEX::IntersectionSimplex( const SIMPLEX& aOther ) const
{
    // Intersection of two simplexes is the union of their half-planes
    std::vector<HALF_PLANE> combined = m_halfPlanes;
    combined.insert( combined.end(), aOther.m_halfPlanes.begin(), aOther.m_halfPlanes.end() );

    return SIMPLEX( combined );
}


int SIMPLEX::IntersectionDimension( const TILE_SHAPE& aOther ) const
{
    auto intersection = Intersection( aOther );

    if( !intersection )
        return -1;  // Empty

    const SIMPLEX* s = dynamic_cast<const SIMPLEX*>( intersection.get() );
    if( s )
    {
        if( s->m_vertices.empty() )
            return -1;
        if( s->m_vertices.size() == 1 )
            return 0;  // Point
        if( s->m_vertices.size() == 2 )
            return 1;  // Line segment
        return 2;  // Area
    }

    return 2;  // Assume area for other shapes
}


VECTOR2I SIMPLEX::Corner( int aIndex ) const
{
    if( aIndex >= 0 && aIndex < (int)m_vertices.size() )
