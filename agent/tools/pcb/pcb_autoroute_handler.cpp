...
bool PCB_AUTOROUTE_HANDLER::CanHandle( const std::string& aToolName ) const
{
    return aToolName == "pcb_autoroute";
}
...
std::string PCB_AUTOROUTE_HANDLER::ExecuteAutoroute( const nlohmann::json& aInput )
{
    ...
    std::string exportResponse = m_sendRequestFn( FRAME_PCB_EDITOR, exportCmd.dump() );
    ...
    std::string importResponse = m_sendRequestFn( FRAME_PCB_EDITOR, importCmd.dump() );
                                           ...
                                           wxEvtHandler* aEventHandler )
{
    ...
    // NOTE: m_sendRequestFn captured by value won't work from another thread (KIWAY is main-thread only)
    // So we capture it and use CallAfter to execute on main thread
    auto sendRequestFn = m_sendRequestFn;
